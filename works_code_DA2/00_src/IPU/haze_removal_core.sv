...
module haze_removal_core #(
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!recovery_enable) begin
                ...
                `ifdef SIM_MODE
                if (recovery_count == 0)
                    $display("DBG @%0t: FIRST recovery_count tick! rec_valid_d1=%b", $time, rec_valid_d1);
                if (recovery_count[11:0] == 12'd0)
                    $display("DBG @%0t: rec_valid_d1 count=%0d/%0d", $time, recovery_count, FramePixels);
                `endif
                if (recovery_count == FramePixels - 1) begin
                    recovery_done  <= 1'b1;
                    recovery_count <= 0;
                    `ifdef SIM_MODE
                    $display("DBG @%0t: recovery_done!", $time);
                    `endif
                end else begin
            ...
            `ifdef SIM_MODE
            if (recovery_enable && !rec_valid && (recovery_count == 0))
                if ($time > 400000 && $time < 400200)
                    $display("DBG @%0t: recovery_enable=1 rec_valid=0 fuse_valid_direct=%b fuse_direct_valid_d2=%b fuse_direct_valid_d1=%b",
                             $time, fuse_valid_direct, fuse_direct_valid_d2, fuse_direct_valid_d1);
            `endif
    ...
    `ifdef SIM_MODE
    // Debug: count post_frame_href pulses
    integer dbg_href_count;
    initial dbg_href_count = 0;
    always @(posedge clk) begin
        if (post_frame_href && post_frame_clken) begin
            dbg_href_count <= dbg_href_count + 1;
            if (dbg_href_count[11:0] == 12'd0)
                $display("DBG_WR @%0t: post_frame_href count=%0d rec_valid_d1=%b rec_href_aligned=%b rec_clken_aligned=%b",
                         $time, dbg_href_count, rec_valid_d1, rec_href_aligned, rec_clken_aligned);
        end
        if (recovery_enable && rec_valid_d1 && !rec_href_aligned && (dbg_href_count < 10))
            $display("DBG_WR @%0t: rec_valid_d1=1 but rec_href_aligned=0! count=%0d", $time, dbg_href_count);
    end
    `endif
