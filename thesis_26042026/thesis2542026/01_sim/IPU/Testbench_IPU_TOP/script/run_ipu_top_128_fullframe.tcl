...
if {[info exists ::env(LOG_DIR)]} {
    ...
    set log_dir "../01_sim/IPU/Testbench_IPU_TOP/log"
}

file mkdir $log_dir
set ts [clock format [clock seconds] -format "%Y%m%d_%H%M%S"]
set log_file [file join $log_dir "ipu_top_128_${ts}.log"]

transcript file $log_file
transcript on

puts "IPU_TOP automation"
...
if {[info exists ::env(OUT_DIR)]} {
    ...
    set out_dir "../01_sim/IPU/Testbench_IPU_TOP/sim/image/"
}
...
set MEM_DIR "../00_src/IPU/mem"
set TB_DIR  "../01_sim/IPU/Testbench_IPU_TOP/sim"

vlog +acc +define+SIM_MODE +incdir+$SRC_DIR +incdir+$MEM_DIR \
    $SRC_DIR/min3_u8.sv \
    ...
    $MEM_DIR/img_in_bram.v \
    $MEM_DIR/img_out_bram.v \
    $MEM_DIR/ipu_bram_reader.v \
    $MEM_DIR/ipu_bram_writer.v \
    $SRC_DIR/ipu_top.sv \
    $TB_DIR/ipu_top_128_tb.sv

vsim -c ipu_top_128_tb +IMG_IN=$img_in +OUT_DIR=$out_dir -do "run -all; quit -f"
